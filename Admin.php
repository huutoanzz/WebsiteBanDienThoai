...
<?php
    ...
    $stmt->execute();
    if($stmt->rowCount()>0){
        $san_pham = $stmt->fetchAll(PDO::FETCH_BOTH);
    }
	
	if(isset($_POST["ma_sp"])){
		$ma_sp = $_POST["ma_sp"];
		// Thực hiện xóa sản phẩm từ cơ sở dữ liệu
		$sql_delete = "DELETE FROM san_pham WHERE ma_sp = ?";
		$stmt_delete = $pdo->prepare($sql_delete);
		$stmt_delete->execute([$ma_sp]);
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo number_format($sp["don_gia"], 0, ',', '.') ?> VND</td>
							<td><img width="50px" src="image_SP/<?php echo $sp["hinh"]?>"></td>
							<td>
								<form method="post" action="UpdateSP.php?msp=<?php echo $sp["ma_sp"]; ?>">
