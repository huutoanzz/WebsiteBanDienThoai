...
<?php
    ...
    if (isset($_POST["btnUpdate"])) {
        $ma_khach_hang = $_POST["ma_kh"];
        $ten_khach_hang = $_POST["txtCustomerName"];
        $dia_chi = $_POST["txtAddress"];
        $email = $_POST["txtEmail"];
        $so_dien_thoai = $_POST["txtPhone"];
        ...
        if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
            ...
            move_uploaded_file($tmp_name, "image_KH/" . $hinh);
        } else {
            // Nếu không có hình ảnh mới, giữ nguyên hình ảnh cũ
            $hinh = $_POST["old_image"];
        }

        // Thực hiện câu lệnh cập nhật thông tin khách hàng trong cơ sở dữ liệu
        $sql_update = "UPDATE khach_hang SET ten_khach_hang = ?, dia_chi = ?, email = ?, dien_thoai = ?, hinh = ? WHERE ma_khach_hang = ?";
        $stmt_update = $pdo->prepare($sql_update);
        $kq = $stmt_update->execute([$ten_khach_hang, $dia_chi, $email, $so_dien_thoai, $hinh, $ma_khach_hang]);

        if ($kq) {
            $tb = "Cập nhật thông tin khách hàng thành công";
        } else {
            $tb = "Cập nhật thông tin khách hàng không thành công";
        }
    }

    // Lấy thông tin khách hàng để hiển thị trên form
    if (isset($_GET["makh"])) {
        $ma_khach_hang = $_GET["makh"];
        $sql_select = "SELECT * FROM khach_hang WHERE ma_khach_hang = ?";
        $stmt_select = $pdo->prepare($sql_select);
        $stmt_select->execute([$ma_khach_hang]);
        $kh = $stmt_select->fetch(PDO::FETCH_ASSOC);
    }
?>
...
<body class="main-layout">
    ...
    <div id="Content" class="row">
        ...
        <div class="col-9">
            ...
            <br/><br/><br/>
            <form method="post" action="UpdateKH.php" enctype="multipart/form-data">
			<!-- Input cho mã khách hàng -->
			<input type="hidden" name="ma_kh" value="<?php echo $kh['ma_khach_hang']; ?>">

			<!-- Input cho tên khách hàng -->
			<div class="form-group">
				<label for="txtCustomerName">Tên khách hàng:</label>
				<input type="text" class="form-control" id="txtCustomerName" name="txtCustomerName" value="<?php echo $kh['ten_khach_hang']; ?>">
			</div>

			<!-- Input cho email -->
			<div class="form-group">
				<label for="txtEmail">Email:</label>
				<input type="email" class="form-control" id="txtEmail" name="txtEmail" value="<?php echo $kh['email']; ?>">
			</div>

			<!-- Input cho số điện thoại -->
			<div class="form-group">
				<label for="txtPhone">Số điện thoại:</label>
				<input type="tel" class="form-control" id="txtPhone" name="txtPhone" value="<?php echo $kh['dien_thoai']; ?>">
			</div>

			<!-- Input cho địa chỉ -->
			<div class="form-group">
				<label for="txtAddress">Địa chỉ:</label>
				<textarea class="form-control" id="txtAddress" name="txtAddress"><?php echo $kh['dia_chi']; ?></textarea>
			</div>
			...
			<input type="hidden" name="old_image" value="<?php echo $kh['hinh']; ?>">

			<!-- Hiển thị hình ảnh cũ -->
			<div class="form-group">
				<label for="oldImage">Hình ảnh cũ:</label><br>
				<img src="image_KH/<?php echo $kh['hinh']; ?>" alt="Hình ảnh cũ" style="max-width: 200px;"><br>
