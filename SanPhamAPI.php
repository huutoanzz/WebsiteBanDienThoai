...
<?php
$api_url = "https://66359043415f4e1a5e24d37c.mockapi.io/SanPham";
...
if ($response !== false) {
    ...
    if ($data !== null) {
        ...
        if (file_put_contents($file_path, $json_data)) {
            echo "Dữ liệu đã được lưu vào tệp products.json";
        } else {
            echo "Không thể lưu dữ liệu vào file.";
        }
    } else {
        echo "Không thể chuyển đổi dữ liệu JSON từ API.";
    }
} else {
...
if ($json_data !== false) {
    ...
    if ($data !== null) {
        // Dữ liệu đã sẵn sàng để hiển thị
    } else {
        echo "Không thể chuyển đổi dữ liệu JSON.";
    }
} else {
...
<body class="main-layout">
    ...
    <!-- brand -->
   	 <div id="Content" class="row">
			...
			<br/><br/><br/>
			<button class="btn btn-primary" my-5 type="submit"><a class="text-light" href="ThemSP.php">ADD NEW</a></button>
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo number_format($item["Gia"], 0, ',', '.') ?> VND</td>
						 <td><img src="<?php echo htmlspecialchars($item["Hinh"]); ?>" alt="<?php echo htmlspecialchars($item["TenSP"]); ?>"></td>
						<td>
							<form method="post" action="UpdateSP.php?msp=<?php echo htmlspecialchars($item["MaSP"]); ?>">
								<button class="btn btn-success" type="submit">Update</button>
							</form>
							<form method="post" action="Admin.php" style="display: inline-block;">
								<input type="hidden" name="ma_sp" value="<?php echo htmlspecialchars($item["MaSP"]); ?>">
								<button class="btn btn-danger" type="submit" onclick="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này không?')">Delete</button>
							</form>
						</td>
					</tr>
					<?php endforeach; ?>
