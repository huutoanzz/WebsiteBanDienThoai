...
<?php
    ...
    $stmt->execute();
    if($stmt->rowCount()>0){
        $khach_hang = $stmt->fetchAll(PDO::FETCH_BOTH);
    }
	
	if(isset($_POST["ma_khach_hang"])){
		$ma_kh = $_POST["ma_khach_hang"];
		// Thực hiện xóa khách hàng từ cơ sở dữ liệu
		$sql_delete = "DELETE FROM khach_hang WHERE ma_khach_hang = ?";
		$stmt_delete = $pdo->prepare($sql_delete);
		$stmt_delete->execute([$ma_kh]);
    ...
    <!-- brand -->
   	 <div id="Content" class="row">
			...
			<div class="col-9">
				...
				<br/><br/><br/>
				<button class="btn btn-primary" my-5 type="submit"><a class="text-light" href="ThemKH.php">ADD NEW</a></button>
				<table class="table">
					<thead>
						<tr>
							<th>Mã KH</th>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<img width="50px" height="50px" src="image_KH/<?php echo $kh["hinh"]?>" style="object-fit: cover;">
							</td>
							<td>
								<form method="post" action="UpdateKH.php?makh=<?php echo $kh["ma_khach_hang"]; ?>">
									<button class="btn btn-success" type="submit">Update</button>
								</form>
								<form method="post" action="Admin_KH.php" style="display: inline-block;">
									<input type="hidden" name="ma_khach_hang" value="<?php echo $kh["ma_khach_hang"]; ?>">
