...
if (isset($_POST["btnSubmit"])) {
    $ten_kh = $_POST["txtCustomerName"];
    $dia_chi = $_POST["txtAddress"];
    $so_dt = $_POST["txtPhone"];
    $email = $_POST["txtEmail"];
    $hinh_name = $_FILES["image"]["name"];
    $duongdan = "image_KH/"; // Đường dẫn tương đối của hình ảnh sau khi tải lên
    $hinh = $duongdan . $hinh_name;

    // Kiểm tra nếu có tệp hình ảnh được tải lên
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $hinh)) {
        // Nếu di chuyển tệp thành công, thêm thông tin khách hàng vào CSDL
        $sql = "INSERT INTO khach_hang (ten_khach_hang, dia_chi, dien_thoai, email, hinh) VALUES (?, ?, ?, ?, ?)";
        $param = array($ten_kh, $dia_chi, $so_dt, $email, $hinh_name);
        $sta = $pdo->prepare($sql);
        $kq = $sta->execute($param);
        if ($kq) {
            $tb = "Thêm khách hàng thành công";
        } else {
            $tb = "Thêm khách hàng không thành công";
        }
    } else {
        // Nếu di chuyển tệp không thành công, thông báo lỗi
        $tb = "Lỗi khi tải lên hình ảnh";
    }
}
?>
...
<body class="main-layout">
    ...
    <!-- brand -->
   	 <div id="Content" class="row">
            ...
            <br/><br/><br/>
            <form method="post" action="ThemKH.php" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Tên khách hàng</label>
                    <input type="text" class="form-control" placeholder="Nhập tên khách hàng" name="txtCustomerName" id="idCustomerName" required="required"/>
                </div>
                <div class="form-group">
                    <label>Địa chỉ</label>
                    <input type="text" class="form-control" placeholder="Nhập địa chỉ" name="txtAddress" id="idAddress" required="required"/>
                </div>
                <div class="form-group">
                    <label>Số điện thoại</label>
                    <input type="text" class="form-control" placeholder="Nhập số điện thoại" name="txtPhone" id="idPhone" required="required"/>
                </div>
                <div class="form-group">
                    <label>Email</label>
                ...
                <div class="form-group">
                    <label>Hình</label>
                    <input type="file" class="form-control" placeholder="Chọn hình" name="image" />
                </div>
                <div class="form-group">
